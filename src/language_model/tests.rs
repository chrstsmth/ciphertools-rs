...
{
	let a = lang.head.next.node[usize::from(Alphabet::A)].as_ref().unwrap();
	let ab = a.next.node[usize::from(Alphabet::B)].as_ref().unwrap();
	let abc = ab.next.node[usize::from(Alphabet::C)].as_ref().unwrap();
	let abcd = abc.next.node[usize::from(Alphabet::D)].as_ref().unwrap();
	let abcde = abcd.next.node[usize::from(Alphabet::E)].as_ref().unwrap();
